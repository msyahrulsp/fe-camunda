...
channel = create_insecure_channel(
    hostname="localhost", port=26500)  # Create grpc channel
...
@worker.task(task_type="create_order", exception_handler=on_error)
async def create_order(job: Job, order: dict) -> dict:
    order_id = order['id']
    return {"order_id": order_id}

@worker.task(task_type="send_order", exception_handler=on_error)
async def send_order(job: Job, order_id: int) -> dict:
    await zeebe_client.publish_message(ORDER_MSG, "", {"order_id": order_id})
    return {}


@worker.task(task_type="confirm_order", exception_handler=on_error)
...
@worker.task(task_type="send_delivery", exception_handler=on_error)
